...
const fs__default = /*#__PURE__*/_interopDefaultLegacy(fs$j);
...
async function onAfterCompressSettings(options, result, cache) {
  ...
  console.log("xr onAfterCompressSettings ================", options, result);
  const asWebXR = options.packages["xr-plugin"].asWebXR;
  if (asWebXR) {
    if (typeof content.xr === "undefined") {
      content.xr = {};
    }
    content.xr.webxrCompatible = true;
  } else {
    delete content.xr;
  }
...
async function onAfterInit(options, result, cache) {
  console.log("xr onAfterInit ================", options, result);
  const params = result.compileOptions;
  const enableAR = options.packages["xr-plugin"].enableAR;
  if (enableAR) {
    params.cMakeConfig.USE_AR_MODULE = "set(USE_AR_MODULE ON)";
    if (result.compileOptions && result.compileOptions.platform === "android") {
      params.cMakeConfig.USE_AR_AUTO = "set(USE_AR_AUTO ON)";
      params.cMakeConfig.USE_AR_CORE = "set(USE_AR_CORE OFF)";
      params.cMakeConfig.USE_AR_ENGINE = "set(USE_AR_ENGINE OFF)";
    }
  } else {
    if (params && params.cMakeConfig) {
      params.cMakeConfig.USE_AR_MODULE = "set(USE_AR_MODULE OFF)";
      if (result.compileOptions && result.compileOptions.platform === "android") {
        params.cMakeConfig.USE_AR_AUTO = "set(USE_AR_AUTO OFF)";
        params.cMakeConfig.USE_AR_CORE = "set(USE_AR_CORE OFF)";
        params.cMakeConfig.USE_AR_ENGINE = "set(USE_AR_ENGINE OFF)";
      }
    }
...
async function onAfterBuild(options, result, cache) {
  console.log("xr onAfterBuild ================", options, result);
  const enableAR = options.packages["xr-plugin"].enableAR;
  if (enableAR) {
    const src = require$$1.join(__dirname, "../template", result.compileOptions.platform);
    const nativePrjDir = require$$1.join(result.compileOptions.buildDir, "proj");
    if (result.compileOptions.platform === "android") {
      await lib.copy(src, nativePrjDir, { overwrite: true });
      const gradlePropertyPath = require$$1.join(nativePrjDir, "gradle.properties");
      if (lib.existsSync(gradlePropertyPath)) {
        let content = lib.readFileSync(gradlePropertyPath, "utf-8");
        const minimalSDKVersion = parseVersion(content, "PROP_MIN_SDK_VERSION", 21);
        content = content.replace(/PROP_MIN_SDK_VERSION=.*/, `PROP_MIN_SDK_VERSION=${minimalSDKVersion < 26 ? 26 : minimalSDKVersion}`);
        lib.writeFileSync(gradlePropertyPath, content);
      } else {
        console.log(`warning: ${gradlePropertyPath} not found!`);
      }
    } else if (result.compileOptions.platform === "ios") {
      const prjName = result.compileOptions.projectName;
      const cmakeTmpDir = lib.readdirSync(require$$1.join(nativePrjDir, "CMakeFiles")).filter((x) => x.startsWith(prjName))[0];
      const infoPlist = require$$1.join(nativePrjDir, "CMakeFiles", cmakeTmpDir, "Info.plist");
      if (lib.existsSync(infoPlist)) {
        const replacement = `	<key>NSCameraUsageDescription</key>
	<string>Need for AR</string>
`;
        const newlines = [];
        let foundKey = false;
        const lines = lib.readFileSync(infoPlist).toString("utf-8").split("\n");
        for (let i = 0; i < lines.length; i++) {
          if (lines[i].indexOf("NSCameraUsageDescription") >= 0) {
            foundKey = true;
          }
        }
        if (!foundKey) {
          let bAdd = false;
          for (let i = 0; i < lines.length; i++) {
            if (lines[i].match(/CFBundleIdentifier/) && !bAdd) {
              newlines.push(replacement);
              bAdd = true;
            }
            newlines.push(lines[i]);
          }
          lib.writeFileSync(infoPlist, newlines.join("\n"));
        }
      } else {
...
function parseVersion(content, key, def) {
