...
async function afterInit(plateform, options$1, result, cache) {
  ...
  if (android.remotePreView) {
    result.compileOptions.cMakeConfig.USE_XR_REMOTE_PREVIEW = "set(USE_XR_REMOTE_PREVIEW ON)";
  } else {
    result.compileOptions.cMakeConfig.USE_XR_REMOTE_PREVIEW = "set(USE_XR_REMOTE_PREVIEW OFF)";
  }
  result.compileOptions.cMakeConfig.CC_ENABLE_SWAPPY = !!android.swappy;
  result.compileOptions.platform = "xr";
  result.compileOptions.platformName = plateform;
...
async function onAfterInit(options, result, cache) {
  ...
  afterInit("xr-spaces", options, result, cache);
}
async function onBeforeCompressSettings(options, result, cache) {
  beforeCompressSettings("xr-spaces", options, result);
}
async function onAfterCompressSettings(options, result, cache) {
  afterCompressSettings("xr-spaces", options, result);
}
